...
| `task #7 <задача>` | ИИ сам осматривает местность вокруг постройки и делает: `task #7 расчисти задний двор`, `task #7 проложи дорожку к двери`. Без номера — перед игроком. Только сервер Java |
...
Для `task` модели даны инструменты (`mcai/tasks.py`), всё в координатах постройки:
`get_build` (запрос, габариты, код), `terrain_in_build` (природа, наползшая на постройку, — карта высот;
блоки самой постройки сюда не попадают), `scan_terrain` (карта высот земли, деревья, вода) и
`get_blocks` (какие блоки в объёме). Модель сначала смотрит, потом пишет обычный скрипт.

В скриптах есть `clear_terrain(x1, y1, z1, x2, y2, z2)` (только сервер Java): мост убирает в этом объёме
только природные блоки и никогда — блоки построек; заодно возвращает блоки постройки, испорченные
насыпавшейся землёй (трава под землёй становится землёй).

Рассуждения модели (`LLM_THINKING`): `off` (по умолчанию), `final` — только на финальном шаге задачи,
`on` — всегда. Qwen3 может рассуждать тысячи токенов и не уложиться в лимит ответа; на задаче «срезать
гору во дворе» режим `off` был быстрее всех (15 с) и расчистил двор полностью, не задев постройку.
Мир читает сайдкар `world-reader` в поде сервера: мост делает `save-all flush`, сайдкар читает
файлы регионов (`mcai/anvil.py`, `mcai/world_server.py`). Результат задачи — отдельная запись
«задача для #7»; удаление уберёт поставленные блоки, но срезанную землю не вернёт.
